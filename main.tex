...
\begin{itemize}
  ...
  \item separate auxillary system
...
\subsubsection{Communication between car and cloud}\label{sssec:car-cloud}
...
There will be multiple streams of live data going between the cloud and the car continuously. This will consist of navigation updates and API calls. The software within our car has standard API requests that will always have a response from our cloud, We decided that the cloud would have a general API request which would provide a link to the external APIs that we use. This is so that if the external APIs are unavailable we can always just update the cloud to provide a new link to a different external API. These services are mentioned in more detail in section \ref{sssec:devices-APIs}.
...
Users can also livestream the car cameras feed to their phones through our app. This communication will be dynamically encoded by the cars computing unit and then transmitted using RTSP. This is to make best use of the connection of the car by adjusting the bitrate of video based on the cars bandwidth.
...
%I think we briefly touched on it being dynamically encoded by the car to make best use of the connection

\subsubsection{Communication between phone and cloud}
Files on the phone are synchronised with the cloud in the same way files on the car are synchronised. Which is mentioned above in section \ref{sssec:car-cloud}.
...
The car system and the mobile app gather alot of information from external APIs. This information ranges from data required for navigation such as roadwork information to getting the latest weather forecasts.

Many sections of the ov
...
We leverage the implicit authentication flow from TLS backed by per-user certificates. Each instance of 
...
We also performed some research to find out exactly what kind of features are currently offered by SatNav systems.
% Complete above paragraph
\begin{itemize}
  \item Questionnaires
...
\subsubsection{Algorithm justifications}
