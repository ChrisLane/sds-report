...
When a login attempt is generated, a notification is sent to any device that the user is logged in on asking to authenticate the request which after logging in and accepting will fulfil the request, bypassing the need to enter the code on the new device. 
...
learnt user habits \& preferences, applying lower weight to road types preferred by the driver such as if they frequently divert through smaller roads, and applying a higher weight to road types that tend to be avoided by the driver as well as preference of `fastest route' or `shortest dis' ;
...
In order to suggest the optimal route for a specific user, the navigation system will make use of a Recommender System, which is a piece of software that uses a Machine Learning algorithm in order to guess a rating for a small set of routes based upon user habits. The 3 routes with the highest ratings will be 
Reccommender system
...
-- Talk about how this prototype matches the design choice in previous section
...
We looked at popular android apps to decide on a layout which the user would be familiar with. This was deemed to be layouts with large buttons and text describing what the buttons do, which was confirmed the best by our QOC discussions.
...
On the home screen (left most interface in Figure \ref{figure-app-hifi}) we have a simple list of functions, these are selectable buttons which have labels on them. This list is scrollable so more features can easily be added in the future. We tried to keep this design to the best QOC option that we found for laying out the main menu.
