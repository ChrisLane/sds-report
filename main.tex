...
The car and the cloud communicate with each other mainly to synchronise information. The car sends hashes and timestamps of its files to the server which compares them with the latest versions on the server. The files which are different are sent back to the car for synchronisation.
...
-- standard Bluetooth usages such as sharing contacts/music over Bluetooth/taking calls etc.
...
As part of the brainstorming process, we investigated current technology to see how the current navigation systems were perceived by various people. The first part of this research takes the form of a questionnaire we created to gain opinions on both current technology and some of our ideas.
\begin{itemize}
  \item Questionnaires
